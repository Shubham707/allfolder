...
class Userdetail extends CI_Controller
{
	...
	{
		    $sql="select * from claimBitcandy";
        $query['listing']=$this->db->query($sql)->result();
        $this->load->view('adminpanel/userdetail',$query); 
    ...
    public function userdata()
    {
      ...
      $data .='<table id="datatable-checkbox" class="table table-striped table-bordered bulk_action">
                          ...
                          <tbody>';
                          $i=1;
          $detail=$this->db->query('select * from claimBitcandy')->result();
                    //print_r($detail); die();
          foreach($detail as $transaction)
            {
            	...
            	$data .='<tr>';
			        $data .='<td>'.$i.'</td>
                			 <td><input type="checkbox">'.$transaction->user_id.'</td>
						     <td>'.$transaction->amount_of_bch.'</td>
                <td>'.$transaction->bch_address.'</td><td>';

						  $data .='<a class="btn btn-sm btn-success" title="User Account unblock" href="#"  onclick="userenabledisable(\''.$transaction->user_id.'\',1);"><i class="fa fa-eye"></i></a>';
              ...
              $data .='<a class="btn btn-sm btn-success" title="User  Two factor Authentication unblock  "  href="#" onclick="usertfaenabledisable(\''.$transaction->user_id.'\',1);"><i class="fa fa-lock"></i></a>';
                 
              $data .='<a class="btn btn-sm btn-danger" title="User  Two factor Authentication block  "  href="#" onclick="usertfaenabledisable(\''.$transaction->user_id.'\',0);"><i class="fa fa-unlock"></i></a>';
                 


              $data .='<a class="btn btn-sm btn-warning" title="User Transactions details " href="#" onclick="usertransactionlist(\''.$transaction->user_id.'\');" data-toggle="modal" data-target="#myModal"><i class="fa fa-file-text"></i></a>';
